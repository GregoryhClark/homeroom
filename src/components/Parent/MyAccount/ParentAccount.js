...
class ParentAccount extends Component{
    ...
    componentWillMount(){
        this.setState({editParent:this.props.user})
    }
    ...
    handleAccountSave(){
        let  {editParent} = this.state;
        this.setState({saveStatus: 'pending'})
        this.props.updateUser(editParent).then(update => {
            this.setState({saveStatus: true})
        })
    }
    render(){
        let {first_name, email, phone_number} = this.state.editParent;
        let {saveStatus} = this.state;
        let currentParent = this.props.user;
        return(
            <div className="student-account">
                <Container type="container">
                    <Titles type="titles">{`${currentParent.first_name}'s Settings`}</Titles>
...
export default connect(mapStateToProps, {getUser, updateUser})(ParentAccount);
