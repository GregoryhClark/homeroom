...
class TeachersAccount extends Component{
    constructor() {
        super()
        this.state = {
          editTeacher: {
    ...
    componentWillMount(){
        this.setState({editTeacher:this.props.user})
    }
    ...
    handleAccountSave(){
        let  {editTeacher} = this.state;
        this.setState({saveStatus: 'pending'})
        this.props.updateUser(editTeacher).then(update => {
            this.setState({saveStatus: true})
        })
    }
    render(){
        let {first_name, email, phone_number} = this.state.editTeacher;
...
export default connect(mapStateToProps, {getUser, updateUser})(TeachersAccount);
