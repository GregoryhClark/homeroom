...
class AdminsAccount extends Component{
    ...
    componentWillMount(){
        this.setState({editAdmin:this.props.user})
    }
    handleUpdateState(e, field){
        const editAdmin = {...this.state.editAdmin}
        editAdmin[field] = e.target.value;
        this.setState({editAdmin})
    }
    handleAccountSave(){
        let  {editAdmin} = this.state;
        this.setState({saveStatus: 'pending'})
        this.props.updateUser(editAdmin).then(update => {
            this.setState({saveStatus: true})
        })
    }
    render(){
        let {first_name, email, phone_number} = this.state.editAdmin;
        let {saveStatus} = this.state;
        let currentAdmin = this.props.user;
        return(
            <div id="admin-account" className="student-account">
                <Container type="container">
                    <Titles type="titles">{`${currentAdmin.first_name}'s Settings`}</Titles>
                    <InnerContainer type="innerContainer">
                        ...
                        <ButtonContainer type="buttonContainer">
                            ...
                            <Button type="save" save={this.handleAccountSave}>Update</Button>
                        </ButtonContainer>
                    </InnerContainer>
                </Container>
        {/*
...
export default connect(mapStateToProps, {getUser, updateUser})(AdminsAccount);
